...
#define ptrSize sizeof(uintptr_t)
    ...
    while (1) {
        ...
        kern_return_t ret = vm_read(realtfp0, addr, 0x200, (vm_offset_t*)&buf, &sz);
        ...
        if (*((uint32_t *)buf) == MACHO_HEADER_MAGIC) {
            int ret = vm_read(realtfp0, addr, 0x1000, (vm_offset_t*)&buf, &sz);
            if (ret != KERN_SUCCESS) {
            ...
            for (uintptr_t i=addr; i < (addr+0x2000); i+=(ptrSize)) {
                mach_msg_type_number_t sz;
                int ret = vm_read(realtfp0, i, 0x120, (vm_offset_t*)&buf, &sz);
                ...
                if (!strcmp(buf, "__text") && !strcmp(buf+0x10, "__PRELINK_TEXT")) {
                    ...
                    printf("The fucking Kernel Base is : 0x%llx\n The damn KASLR slide is: 0x%llx\n", addr, addr - 0xfffffff007004000);
...
uint64_t get_KASLR_Slide(){
    kslide = kernel_base - 0xfffffff007004000;
    return kslide; // I use this on the UI.
}
uint tfp0_printout(){
    return realtfp0; //Just return the task port. I use this on the UI.
}
// creds to stek and IBSparkes on this
...
int yolo(){
    ...
    rc = unlink("/jb");
    rc = unlink("/jb/bin/bash");
    rc = unlink("/jb/tar");
    rc = unlink("/etc/dropbear");
    rc = mkdir("/jb", 0755);
    rc = mkdir("/tempo", 0755);
    rc = mkdir("/jb/Library", 0755);
    rc = mkdir("/jb/Library/LaunchDaemons", 0755);
    rc = mkdir("/usr/libexec/filza", 0755);
    rc = execCommand("/jb/bin/rm","-r","/etc/dropbear",0,0,0, 0);
    rc = moveFileFromAppDir("tar","/jb/tar");
    rc = moveFileFromAppDir("sh","/jb/sh");
    rc = moveFileFromAppDir("bash","/jb/bin/bash");
    rc = moveFileFromAppDir("binpack64-256.tar","/jb/binpack64-256.tar");
    rc = chmod("/private", 0777);
    rc = chmod("/private/var", 0777);
    rc = chmod("/private/var/mobile", 0777);
    rc = chmod("/private/var/mobile/Library", 0777);
    rc = chmod("/private/var/mobile/Library/Preferences", 0777);
    rc = chmod("/jb/tar", 0755);
    rc = chmod("/jb/binpack64-256.tar", 0755);
    rc = chmod("/jb/sh", 0755);
    rc = chmod("/jb/bin/bash", 777);
    rc = chmod("/jb/usr/local/bin/dropbear", 777);
    rc = execCommand("/jb/tar","-C","/jb","-xvf","/jb/binpack64-256.tar",0, 0);
    rc = execprog("/jb/dropbear", (const char **)&(const char*[]){ "/jb/dropbear", "-F","-R", "--shell /jb/bin/bash", "-p", "27.0.0.1:22", NULL });
    printf("\n\n\n\n\n\n\n\n\n\n\n\n MMMMMMMMM SHAI HULUD! (Jailbreak Complete!) \n\n\n\n\n\n\n\n\n\n");
    return 0;
}
